...
#set output
OUTPUT = "~/tscr/kai/projects/01_SCC_scRNA/results/samples/cohort/patient7/7a/"
```

# Load files

```{r}
data_dir = "~/tscr/Allgemeine Daten/10x Genomics/data/PEI04_A006850061_SCC/cellranger/SID121693/outs/"

#raw data containing gene expression and Hashtag matrices
patient7a.data <- Read10X(paste0(data_dir,"raw_feature_bc_matrix/"))
#Setup Seurat object for gene Expression
patient7a <- CreateSeuratObject(patient7a.data$`Gene Expression`, project = "patient7a")
#Adding HTO data as an independent assay
patient7a[["HTO"]] <- CreateAssayObject(counts = patient7a.data$`Antibody Capture`)
...
patient7a[["percent.mito"]] <- PercentageFeatureSet(patient7a, pattern = "^MT-") 
...
hkgenes.found <- which(toupper(rownames(patient7a)) %in% toupper(hkgenes))
# sum of hk genes for each cell
hk_per_cell <- Matrix::colSums(GetAssayData(patient7a)[hkgenes.found, ] > 0)
patient7a <- AddMetaData(object = patient7a, metadata = hk_per_cell, col.name = "n.exp.hkgenes")
...
barcode_metrics <- data.frame(barcodes = names(patient7a$nCount_RNA),
                              nCount = patient7a$nCount_RNA,
                              nFeature = patient7a$nFeature_RNA,
                              percent.mito = patient7a$percent.mito,
                              n.exp.hkgenes = patient7a$n.exp.hkgenes)

cumplotdata <- data.frame(x=1:1000, y= unlist(lapply(1:1000,function(x) sum(patient7a$nCount_RNA < x) / length(patient7a$nCount_RNA < x))))
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-cumplot-log10.svg"),cumplot.log10)
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsGENE-mito.svg"),UMIvsGENE.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsGENE-hk.svg"),UMIvsGENE.hk)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-HKvsMITO-UMI.svg"),HKvsMITO.UMI)
```
...
patient7a.filt <- subset(patient7a,
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsGENE-filt.svg"),UMIvsGENE.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsGENE-cells-mito.svg"),UMIvsGENE.cells.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsGENE-notcells-mito.svg"),UMIvsGENE.notcells.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsMITO-countfilt-hk.svg"),UMIvsMITO.cf.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsHK-countfilt-mito.svg"),UMIvsHK.cf.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-HKvsMITO-countfilt-UMI.svg"),HKvsMITO.cf.UMI)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsMITO-filt.svg"),UMIvsMITO.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsMITO-cells-hk.svg"),UMIvsMITO.cells.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsMITO-notcells-hk.svg"),UMIvsMITO.notcells.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsHK-filt.svg"),UMIvsHK.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsHK-cells-mito.svg"),UMIvsHK.cells.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-UMIvsHK-notcells-mito.svg"),UMIvsHK.notcells.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-HKvsMITO-filt.svg"),HKvsMITO.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-HKvsMITO-cells-UMI.svg"),HKvsMITO.cells.UMI)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-filtering-HKvsMITO-notcells-UMI.svg"),HKvsMITO.notcells.UMI)
```
...
```{r}
rownames(patient7a.filt@assays$HTO@counts) <- c("7.1","7.2","7.3")
rownames(patient7a.filt@assays$HTO@data) <- c("7.1","7.2","7.3")

#names in HTO matrix
HTO1 <- paste0("hto_",rownames(patient7a.filt@assays$HTO)[1])
HTO2 <- paste0("hto_",rownames(patient7a.filt@assays$HTO)[2])
HTO3 <- paste0("hto_",rownames(patient7a.filt@assays$HTO)[3])
```

## Normalize HTO expression matrix

```{r}
patient7a.filt <- NormalizeData(patient7a.filt, assay = "HTO", normalization.method = "CLR") #centered log-ratio transformation
```
...
htoexpression.norm.df <- FetchData(object = patient7a.filt, vars = c(HTO1,HTO2,HTO3),slot = "data")
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-123.svg"),HTOexpr123.norm)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-132.svg"),HTOexpr132.norm)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-231.svg"),HTOexpr231.norm)
```
...
patient7a.filt.quant <- list()
patient7a.filt.quant.table <- list()
for (quant in seq(0.5,0.99,0.01)) {
  #Demultiplexing
  patient7a.filt.quant[[as.character(quant)]] <- patient7a.filt
  patient7a.filt.quant[[as.character(quant)]] <- HTODemux(patient7a.filt.quant[[as.character(quant)]], assay = "HTO", positive.quantile = quant)
  patient7a.filt.quant.table[[as.character(quant)]] <- table(patient7a.filt.quant[[as.character(quant)]]$HTO_classification.global)
}
rm(patient7a.filt.quant)

demux.df <- data.frame(quantile=names(patient7a.filt.quant.table),
                       Doublet=unlist(lapply(patient7a.filt.quant.table,function(x) x[1])),
                       Negative=unlist(lapply(patient7a.filt.quant.table,function(x) x[2])),
                       Singlet=unlist(lapply(patient7a.filt.quant.table,function(x) x[3])))
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOquant.svg"),HTOquant, width=10)
...
patient7a.filt <- HTODemux(patient7a.filt, assay = "HTO", positive.quantile = 0.99, seed = 100)
patient7a.filt$hash.ID.hpq <- factor(patient7a.filt$hash.ID,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
table(patient7a.filt$HTO_classification.global)
...
patient7a.filt <- HTODemux(patient7a.filt, assay = "HTO", positive.quantile = posquantile, seed = 100)
patient7a.filt$hash.ID.lpq <- factor(patient7a.filt$hash.ID,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
table(patient7a.filt$HTO_classification.global)
```
...
```{r}
htoexpression.norm.df$hash.ID.hpq <- patient7a.filt$hash.ID.hpq
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-12-hpq.svg"),HTOexpr12.norm.hpq)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-13-hpq.svg"),HTOexpr13.norm.hpq)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-23-hpq.svg"),HTOexpr23.norm.hpq)

htoexpression.norm.df$hash.ID.lpq <- patient7a.filt$hash.ID.lpq
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-12-lpq.svg"),HTOexpr12.norm.lpq)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-13-lpq.svg"),HTOexpr13.norm.lpq)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-23-lpq.svg"),HTOexpr23.norm.lpq)
```

UMAP Visualization of HTO Expression

```{r HTO-umap}
patient7a.filt <- ScaleData(patient7a.filt,
                           assay = "HTO",
                           features = rownames(GetAssayData(patient7a.filt,assay = "HTO")))
patient7a.filt <- RunPCA(patient7a.filt,
                        assay = "HTO", 
                        features = rownames(GetAssayData(patient7a.filt,assay = "HTO")), 
                        approx = FALSE,
                        reduction.name = "pca_HTO")
patient7a.filt <- RunUMAP(patient7a.filt,
                         dims = 1:length(rownames(GetAssayData(patient7a.filt,assay = "HTO"))),
                         assay = "HTO", 
...
HTOumap.HTOexpr <- FeaturePlot(patient7a.filt,
                               pt.size = 1,
                               reduction = "umap_HTO",
                               features = rownames(GetAssayData(patient7a.filt,assay = "HTO"))) +
  theme_jb_nogrid()
patient7a.filt$nCount_RNA_log10 <- log10(patient7a.filt$nCount_RNA)
patient7a.filt$nFeature_RNA_log10 <- log10(patient7a.filt$nFeature_RNA)

HTOumap.QC <- FeaturePlot(patient7a.filt,
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTO-UMAP-HTOexpr.svg"),HTOumap.HTOexpr,width=5,height=5)
lapply(names(HTOumap.QC), function(x) ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTO-UMAP-",x,".svg"),HTOumap.QC[[x]],width=5,height=5))
HTOumap.HTOexpr
HTOumap.QC
```

```{r}
HTOumap.hashID.hpq <- DimPlot(patient7a.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
HTOumap.hashID.lpq <- DimPlot(patient7a.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTO-UMAP-hashID-hpq.svg"),HTOumap.hashID.hpq,width=5,height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTO-UMAP-hashID-lpq.svg"),HTOumap.hashID.lpq,width=5,height=5)
...
HTOcounts <- FetchData(object = patient7a.filt, vars = c(HTO1,HTO2,HTO3),slot = "counts")
...
patient7a.filt$hash.ID.man <- factor(HTOcounts$manuelfilt,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
```
...
```{r}
htoexpression.norm.df$hash.ID.man <- patient7a.filt$hash.ID.man
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-12-man.svg"),HTOexpr12.norm.man)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-13-man.svg"),HTOexpr13.norm.man)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTOexpr-norm-23-man.svg"),HTOexpr23.norm.man)
```

```{r}
HTOumap.hashID.man <- DimPlot(patient7a.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-demux-HTO-UMAP-hashID-man.svg"),HTOumap.hashID.man,width=5,height=5)
...
patient7a.filt <- NormalizeData(patient7a.filt, 
...
patient7a.filt <- FindVariableFeatures(patient7a.filt, 
                                       assay = "RNA",
...
allgenes <- rownames(patient7a.filt)
patient7a.filt <- ScaleData(patient7a.filt, 
                            features = allgenes, 
                            do.scale = T,
                            do.center = T,
...
```{r}
patient7a.filt <- CellCycleScoring(patient7a.filt,
...
```{r}
patient7a.filt <- SCTransform(patient7a.filt, 
                              ncells = ncol(patient7a.filt),
                              assay="RNA", #default
...
patient7a.filt <- RunPCA(patient7a.filt, 
                         ...
                         features = VariableFeatures(patient7a.filt,assay = "SCT"), #default
...
PCAvar.SCT <- patient7a.filt@reductions$pca_SCT@stdev^2 / sum(matrixStats::rowVars(as.matrix(patient7a.filt@assays$SCT@scale.data)[VariableFeatures(patient7a.filt,assay = "SCT"),]))

#PCA plot
PCAplot.SCT <- DimPlot(patient7a.filt, reduction = "pca_SCT", dims = c(1,2), group.by = "hash.ID") + 
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-PCA.svg"),PCAplot.SCT, width = 5, height = 5)
# without legends
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-PCA-noleg.svg"),PCAplot.SCT + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-PCA-elbow.svg"),PCAelbowplot.SCT, width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-PCA-elbow-noleg.svg"),PCAelbowplot.SCT + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-PCA-variancesum.svg"),PCAsumplot.SCT, width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-PCA-variancesum-noleg.svg"),PCAsumplot.SCT + theme(legend.position = "none") + labs(title = NULL),width = 5, height = 5)
...
#based on PCA
patient7a.filt <- FindNeighbors(patient7a.filt, 
                                dims = 1:numberofPCs, 
                                reduction = "pca_SCT",
                                assay = "SCT")
patient7a.filt <- FindClusters(patient7a.filt, 
                               resolution = 1,
                               random.seed=100)

patient7a.filt$seurat_clusters_all_SCT_PCA <- patient7a.filt$seurat_clusters
patient7a.filt$seurat_clusters <- NULL 
```
...
```{r}
UMAP.SCT.clusPCA <- DimPlot(patient7a.filt, 
                            reduction = "umap_SCT", 
                            pt.size = 1, 
                            label = T, 
...
UMAP.SCT.hashIDman <- DimPlot(patient7a.filt, 
                              reduction = "umap_SCT", 
                              pt.size = 1, 
                              label = F, 
                              group.by = "hash.ID.man") +
...
UMAP.SCT.QC <- FeaturePlot(patient7a.filt, 
...
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-UMAP-clusPCA-noleg.svg"),UMAP.SCT.clusPCA + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-UMAP-hashIDman.svg"),UMAP.SCT.hashIDman, width=5, height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-UMAP-hashIDman-noleg.svg"),UMAP.SCT.hashIDman + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-UMAP-QC.svg"),UMAP.SCT.QC, width=10, height=15)
...
```{r}
UMAP.SCT.marker <- lapply(names(celltypemarkers), function(x) FeaturePlot_markers_comb(patient7a.filt,celltypemarkers[[x]],reduction = "umap_SCT"))
names(UMAP.SCT.marker) <- names(celltypemarkers)

lapply(names(UMAP.SCT.marker), function(x) ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-UMAP-marker-",x,".svg"),UMAP.SCT.marker[[x]], width=ifelse(length(celltypemarkers[[x]])>1,10,5), height=5*ceiling(length(celltypemarkers[[x]])/2)))
...
```{r umap-celltype}
Idents(patient7a.filt) <- patient7a.filt$seurat_clusters_all_SCT_PCA
patient7a.filt <- RenameIdents(patient7a.filt,
                               "0" = "T-Cells",
                               "1" = "T-Cells",
                               "2" = "T-Cells",
                               "3" = "B-Cells",
                               ...
                               "8" = "myeloid cells",
                               "9" = "T-Cells",
                               "10" = "cycling Immune cells",
                               "11" = "Plasmablasts",
                               "12" = "pDCs",
                               "13" = "RBCs"
)
patient7a.filt$CellType <- Idents(patient7a.filt)
```

## UMAP

```{r}
UMAP.SCT.celltype <- DimPlot(patient7a.filt,
                             reduction = "umap_SCT", 
                             pt.size = 1, 
                             label = T, 
                             group.by = "CellType") + 
  labs(x="UMAP_1",y="UMAP_2") +
  theme_jb_nogrid()
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-UMAP-celltype.svg"),UMAP.SCT.celltype, width=5, height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient7a-all-SCT-UMAP-celltype-noleg.svg"),UMAP.SCT.celltype + theme(legend.position = "none") + labs(title = NULL), width=5, height=5)
UMAP.SCT.celltype
```
...
save(file = paste0(OUTPUT,"RData/patient7a.RData"),patient7a.filt)
#sessionInfo
writeLines(capture.output(sessionInfo()), paste0(OUTPUT,"RData/patient7a-preprocessing-sessionInfo.txt"))
```
