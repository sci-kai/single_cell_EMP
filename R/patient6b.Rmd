...
#set output
OUTPUT = "~/tscr/kai/projects/01_SCC_scRNA/results/samples/cohort/patient6/6b/"
```

# Load files

```{r}
data_dir = "~/tscr/Allgemeine Daten/10x Genomics/data/PEI04_A006850061_SCC/cellranger/SID121691/outs/"

#raw data containing gene expression and Hashtag matrices
patient6b.data <- Read10X(paste0(data_dir,"raw_feature_bc_matrix/"))
#Setup Seurat object for gene Expression
patient6b <- CreateSeuratObject(patient6b.data$`Gene Expression`, project = "patient6b")
#Adding HTO data as an independent assay
patient6b[["HTO"]] <- CreateAssayObject(counts = patient6b.data$`Antibody Capture`)
...
patient6b[["percent.mito"]] <- PercentageFeatureSet(patient6b, pattern = "^MT-") 
...
hkgenes.found <- which(toupper(rownames(patient6b)) %in% toupper(hkgenes))
# sum of hk genes for each cell
hk_per_cell <- Matrix::colSums(GetAssayData(patient6b)[hkgenes.found, ] > 0)
patient6b <- AddMetaData(object = patient6b, metadata = hk_per_cell, col.name = "n.exp.hkgenes")
...
barcode_metrics <- data.frame(barcodes = names(patient6b$nCount_RNA),
                              nCount = patient6b$nCount_RNA,
                              nFeature = patient6b$nFeature_RNA,
                              percent.mito = patient6b$percent.mito,
                              n.exp.hkgenes = patient6b$n.exp.hkgenes)

cumplotdata <- data.frame(x=1:1000, y= unlist(lapply(1:1000,function(x) sum(patient6b$nCount_RNA < x) / length(patient6b$nCount_RNA < x))))
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-cumplot-log10.svg"),cumplot.log10)
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsGENE-mito.svg"),UMIvsGENE.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsGENE-hk.svg"),UMIvsGENE.hk)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-HKvsMITO-UMI.svg"),HKvsMITO.UMI)
```
...
patient6b.filt <- subset(patient6b,
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsGENE-filt.svg"),UMIvsGENE.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsGENE-cells-mito.svg"),UMIvsGENE.cells.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsGENE-notcells-mito.svg"),UMIvsGENE.notcells.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsMITO-countfilt-hk.svg"),UMIvsMITO.cf.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsHK-countfilt-mito.svg"),UMIvsHK.cf.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-HKvsMITO-countfilt-UMI.svg"),HKvsMITO.cf.UMI)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsMITO-filt.svg"),UMIvsMITO.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsMITO-cells-hk.svg"),UMIvsMITO.cells.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsMITO-notcells-hk.svg"),UMIvsMITO.notcells.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsHK-filt.svg"),UMIvsHK.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsHK-cells-mito.svg"),UMIvsHK.cells.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-UMIvsHK-notcells-mito.svg"),UMIvsHK.notcells.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-HKvsMITO-filt.svg"),HKvsMITO.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-HKvsMITO-cells-UMI.svg"),HKvsMITO.cells.UMI)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-filtering-HKvsMITO-notcells-UMI.svg"),HKvsMITO.notcells.UMI)
```
...
```{r}
rownames(patient6b.filt@assays$HTO@counts) <- c("6.4","6.5","6.6")
rownames(patient6b.filt@assays$HTO@data) <- c("6.4","6.5","6.6")

#names in HTO matrix
HTO1 <- paste0("hto_",rownames(patient6b.filt@assays$HTO)[1])
HTO2 <- paste0("hto_",rownames(patient6b.filt@assays$HTO)[2])
HTO3 <- paste0("hto_",rownames(patient6b.filt@assays$HTO)[3])
```

## Normalize HTO expression matrix

```{r}
patient6b.filt <- NormalizeData(patient6b.filt, assay = "HTO", normalization.method = "CLR") #centered log-ratio transformation
```
...
htoexpression.norm.df <- FetchData(object = patient6b.filt, vars = c(HTO1,HTO2,HTO3),slot = "data")
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOexpr-norm-123.svg"),HTOexpr123.norm)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOexpr-norm-132.svg"),HTOexpr132.norm)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOexpr-norm-231.svg"),HTOexpr231.norm)
```
...
patient6b.filt.quant <- list()
patient6b.filt.quant.table <- list()
for (quant in seq(0.5,0.99,0.01)) {
  #Demultiplexing
  patient6b.filt.quant[[as.character(quant)]] <- patient6b.filt
  patient6b.filt.quant[[as.character(quant)]] <- HTODemux(patient6b.filt.quant[[as.character(quant)]], assay = "HTO", positive.quantile = quant)
  patient6b.filt.quant.table[[as.character(quant)]] <- table(patient6b.filt.quant[[as.character(quant)]]$HTO_classification.global)
}
rm(patient6b.filt.quant)

demux.df <- data.frame(quantile=names(patient6b.filt.quant.table),
                       Doublet=unlist(lapply(patient6b.filt.quant.table,function(x) x[1])),
                       Negative=unlist(lapply(patient6b.filt.quant.table,function(x) x[2])),
                       Singlet=unlist(lapply(patient6b.filt.quant.table,function(x) x[3])))
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOquant.svg"),HTOquant, width=10)
...
patient6b.filt <- HTODemux(patient6b.filt, assay = "HTO", positive.quantile = 0.99, seed = 100)
patient6b.filt$hash.ID.hpq <- factor(patient6b.filt$hash.ID,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
table(patient6b.filt$HTO_classification.global)
```

Quality control

```{r}
htoexpression.norm.df$hash.ID.hpq <- patient6b.filt$hash.ID.hpq
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOexpr-norm-12-hpq.svg"),HTOexpr12.norm.hpq)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOexpr-norm-13-hpq.svg"),HTOexpr13.norm.hpq)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOexpr-norm-23-hpq.svg"),HTOexpr23.norm.hpq)
...
```{r HTO-umap}
patient6b.filt <- ScaleData(patient6b.filt,
                            assay = "HTO",
                            features = rownames(GetAssayData(patient6b.filt,assay = "HTO")))
patient6b.filt <- RunPCA(patient6b.filt,
                         assay = "HTO", 
                         features = rownames(GetAssayData(patient6b.filt,assay = "HTO")), 
                         approx = FALSE,
                         reduction.name = "pca_HTO")
patient6b.filt <- RunUMAP(patient6b.filt,
                          dims = 1:length(rownames(GetAssayData(patient6b.filt,assay = "HTO"))),
                          assay = "HTO", 
...
HTOumap.HTOexpr <- FeaturePlot(patient6b.filt,
                               pt.size = 1,
                               reduction = "umap_HTO",
                               features = rownames(GetAssayData(patient6b.filt,assay = "HTO"))) +
  theme_jb_nogrid()
patient6b.filt$nCount_RNA_log10 <- log10(patient6b.filt$nCount_RNA)
patient6b.filt$nFeature_RNA_log10 <- log10(patient6b.filt$nFeature_RNA)

HTOumap.QC <- FeaturePlot(patient6b.filt,
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTO-UMAP-HTOexpr.svg"),HTOumap.HTOexpr,width=5,height=5)
lapply(names(HTOumap.QC), function(x) ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTO-UMAP-",x,".svg"),HTOumap.QC[[x]],width=5,height=5))
HTOumap.HTOexpr
HTOumap.QC
```

```{r}
HTOumap.hashID.hpq <- DimPlot(patient6b.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTO-UMAP-hashID-hpq.svg"),HTOumap.hashID.hpq,width=5,height=5)
...
HTOcounts <- FetchData(object = patient6b.filt, vars = c(HTO1,HTO2,HTO3),slot = "counts")
...
patient6b.filt$hash.ID.man <- factor(HTOcounts$manuelfilt,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
```
...
```{r}
htoexpression.norm.df$hash.ID.man <- patient6b.filt$hash.ID.man
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOexpr-norm-12-man.svg"),HTOexpr12.norm.man)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOexpr-norm-13-man.svg"),HTOexpr13.norm.man)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTOexpr-norm-23-man.svg"),HTOexpr23.norm.man)
```

```{r}
HTOumap.hashID.man <- DimPlot(patient6b.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-demux-HTO-UMAP-hashID-man.svg"),HTOumap.hashID.man,width=5,height=5)
...
patient6b.filt <- NormalizeData(patient6b.filt, 
...
patient6b.filt <- FindVariableFeatures(patient6b.filt, 
                                       assay = "RNA",
...
allgenes <- rownames(patient6b.filt)
patient6b.filt <- ScaleData(patient6b.filt, 
                            features = allgenes, 
                            do.scale = T,
                            do.center = T,
...
```{r}
patient6b.filt <- CellCycleScoring(patient6b.filt,
...
```{r}
patient6b.filt <- SCTransform(patient6b.filt, 
                              ncells = ncol(patient6b.filt),
                              assay="RNA", #default
...
patient6b.filt <- RunPCA(patient6b.filt, 
                         ...
                         features = VariableFeatures(patient6b.filt,assay = "SCT"), #default
...
PCAvar.SCT <- patient6b.filt@reductions$pca_SCT@stdev^2 / sum(matrixStats::rowVars(as.matrix(patient6b.filt@assays$SCT@scale.data)[VariableFeatures(patient6b.filt,assay = "SCT"),]))

#PCA plot
PCAplot.SCT <- DimPlot(patient6b.filt, reduction = "pca_SCT", dims = c(1,2), group.by = "hash.ID") + 
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-PCA.svg"),PCAplot.SCT, width = 5, height = 5)
# without legends
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-PCA-noleg.svg"),PCAplot.SCT + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-PCA-elbow.svg"),PCAelbowplot.SCT, width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-PCA-elbow-noleg.svg"),PCAelbowplot.SCT + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-PCA-variancesum.svg"),PCAsumplot.SCT, width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-PCA-variancesum-noleg.svg"),PCAsumplot.SCT + theme(legend.position = "none") + labs(title = NULL),width = 5, height = 5)
...
#based on PCA
patient6b.filt <- FindNeighbors(patient6b.filt, 
                                dims = 1:numberofPCs, 
                                reduction = "pca_SCT",
                                assay = "SCT")
patient6b.filt <- FindClusters(patient6b.filt, 
                               resolution = 1,
                               random.seed=100)

patient6b.filt$seurat_clusters_all_SCT_PCA <- patient6b.filt$seurat_clusters
patient6b.filt$seurat_clusters <- NULL 
```
...
```{r}
UMAP.SCT.clusPCA <- DimPlot(patient6b.filt, 
                            reduction = "umap_SCT", 
                            pt.size = 1, 
                            label = T, 
...
UMAP.SCT.hashIDman <- DimPlot(patient6b.filt, 
                              reduction = "umap_SCT", 
                              pt.size = 1, 
                              label = F, 
                              group.by = "hash.ID.man") +
...
UMAP.SCT.QC <- FeaturePlot(patient6b.filt, 
...
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-UMAP-clusPCA-noleg.svg"),UMAP.SCT.clusPCA + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-UMAP-hashIDman.svg"),UMAP.SCT.hashIDman, width=5, height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-UMAP-hashIDman-noleg.svg"),UMAP.SCT.hashIDman + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-UMAP-QC.svg"),UMAP.SCT.QC, width=10, height=15)
...
```{r}
UMAP.SCT.marker <- lapply(names(celltypemarkers), function(x) FeaturePlot_markers_comb(patient6b.filt,celltypemarkers[[x]],reduction = "umap_SCT"))
names(UMAP.SCT.marker) <- names(celltypemarkers)

lapply(names(UMAP.SCT.marker), function(x) ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-UMAP-marker-",x,".svg"),UMAP.SCT.marker[[x]], width=ifelse(length(celltypemarkers[[x]])>1,10,5), height=5*ceiling(length(celltypemarkers[[x]])/2)))
...
```{r umap-celltype}
Idents(patient6b.filt) <- patient6b.filt$seurat_clusters_all_SCT_PCA
patient6b.filt <- RenameIdents(patient6b.filt,
                               "0" = "T-Cells",
                               "1" = "T-Cells",
                               "2" = "B-Cells",
                               "3" = "B-Cells",
...
patient6b.filt$CellType <- Idents(patient6b.filt)
```

## UMAP

```{r}
UMAP.SCT.celltype <- DimPlot(patient6b.filt,
                             reduction = "umap_SCT", 
                             pt.size = 1, 
                             label = T, 
                             group.by = "CellType") + 
  labs(x="UMAP_1",y="UMAP_2") +
  theme_jb_nogrid()
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-UMAP-celltype.svg"),UMAP.SCT.celltype, width=5, height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient6b-all-SCT-UMAP-celltype-noleg.svg"),UMAP.SCT.celltype + theme(legend.position = "none") + labs(title = NULL), width=5, height=5)
UMAP.SCT.celltype
```
...
save(file = paste0(OUTPUT,"RData/patient6b.RData"),patient6b.filt)
#sessionInfo
writeLines(capture.output(sessionInfo()), paste0(OUTPUT,"RData/patient6b-preprocessing-sessionInfo.txt"))
```
