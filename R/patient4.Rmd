...
#set output
OUTPUT = "~/tscr/kai/projects/01_SCC_scRNA/results/samples/cohort/patient4/"
```

# Load files

```{r}
data_dir = "~/tscr/Allgemeine Daten/10x Genomics/data/PEI03_A006850046_MCC-SCC/cellranger/SID114992/outs/"

#raw data containing gene expression and Hashtag matrices
patient4.data <- Read10X(paste0(data_dir,"raw_feature_bc_matrix/"))
#Setup Seurat object for gene Expression
patient4 <- CreateSeuratObject(patient4.data$`Gene Expression`, project = "patient4")
#Adding HTO data as an independent assay
patient4[["HTO"]] <- CreateAssayObject(counts = patient4.data$`Antibody Capture`)
...
patient4[["percent.mito"]] <- PercentageFeatureSet(patient4, pattern = "^MT-") 
...
hkgenes.found <- which(toupper(rownames(patient4)) %in% toupper(hkgenes))
# sum of hk genes for each cell
hk_per_cell <- Matrix::colSums(GetAssayData(patient4)[hkgenes.found, ] > 0)
patient4 <- AddMetaData(object = patient4, metadata = hk_per_cell, col.name = "n.exp.hkgenes")
...
barcode_metrics <- data.frame(barcodes = names(patient4$nCount_RNA),
                              nCount = patient4$nCount_RNA,
                              nFeature = patient4$nFeature_RNA,
                              percent.mito = patient4$percent.mito,
                              n.exp.hkgenes = patient4$n.exp.hkgenes)

cumplotdata <- data.frame(x=1:1000, y= unlist(lapply(1:1000,function(x) sum(patient4$nCount_RNA < x) / length(patient4$nCount_RNA < x))))
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-cumplot-log10.svg"),cumplot.log10)
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsGENE-mito.svg"),UMIvsGENE.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsGENE-hk.svg"),UMIvsGENE.hk)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-HKvsMITO-UMI.svg"),HKvsMITO.UMI)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsGENE-filt.svg"),UMIvsGENE.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsGENE-cells-mito.svg"),UMIvsGENE.cells.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsGENE-notcells-mito.svg"),UMIvsGENE.notcells.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsMITO-countfilt-hk.svg"),UMIvsMITO.cf.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsHK-countfilt-mito.svg"),UMIvsHK.cf.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-HKvsMITO-countfilt-UMI.svg"),HKvsMITO.cf.UMI)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsMITO-filt.svg"),UMIvsMITO.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsMITO-cells-hk.svg"),UMIvsMITO.cells.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsMITO-notcells-hk.svg"),UMIvsMITO.notcells.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsHK-filt.svg"),UMIvsHK.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsHK-cells-mito.svg"),UMIvsHK.cells.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-UMIvsHK-notcells-mito.svg"),UMIvsHK.notcells.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-HKvsMITO-filt.svg"),HKvsMITO.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-HKvsMITO-cells-UMI.svg"),HKvsMITO.cells.UMI)
ggsave(filename = paste0(OUTPUT,"figs/patient4-filtering-HKvsMITO-notcells-UMI.svg"),HKvsMITO.notcells.UMI)
```
...
```{r}
rownames(patient4.filt@assays$HTO@counts) <- c("4.1","4.2","4.3")
rownames(patient4.filt@assays$HTO@data) <- c("4.1","4.2","4.3")
  
#names in HTO matrix
HTO1 <- paste0("hto_",rownames(patient4.filt@assays$HTO)[1])
HTO2 <- paste0("hto_",rownames(patient4.filt@assays$HTO)[2])
HTO3 <- paste0("hto_",rownames(patient4.filt@assays$HTO)[3])
```

## Normalize HTO expression matrix

```{r}
patient4.filt <- NormalizeData(patient4.filt, assay = "HTO", normalization.method = "CLR") #centered log-ratio transformation
```
...
htoexpression.norm.df <- FetchData(object = patient4.filt, vars = c(HTO1,HTO2,HTO3),slot = "data")
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-123.svg"),HTOexpr123.norm)
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-132.svg"),HTOexpr132.norm)
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-231.svg"),HTOexpr231.norm)
```
...
patient4.filt.quant <- list()
patient4.filt.quant.table <- list()
for (quant in seq(0.5,0.99,0.01)) {
  #Demultiplexing
  patient4.filt.quant[[as.character(quant)]] <- patient4.filt
  patient4.filt.quant[[as.character(quant)]] <- HTODemux(patient4.filt.quant[[as.character(quant)]], assay = "HTO", positive.quantile = quant)
  patient4.filt.quant.table[[as.character(quant)]] <- table(patient4.filt.quant[[as.character(quant)]]$HTO_classification.global)
}
rm(patient4.filt.quant)

demux.df <- data.frame(quantile=names(patient4.filt.quant.table),
                       Doublet=unlist(lapply(patient4.filt.quant.table,function(x) x[1])),
                       Negative=unlist(lapply(patient4.filt.quant.table,function(x) x[2])),
                       Singlet=unlist(lapply(patient4.filt.quant.table,function(x) x[3])))
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOquant.svg"),HTOquant, width=10)
...
patient4.filt <- HTODemux(patient4.filt, assay = "HTO", positive.quantile = 0.99, seed = 100)
# patient4.filt$HTO_classification.global_hpq <- patient4.filt$HTO_classification.global
# patient4.filt$HTO_classification_hpq <- patient4.filt$HTO_classification
patient4.filt$hash.ID.hpq <- factor(patient4.filt$hash.ID,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
table(patient4.filt$HTO_classification.global)
...
patient4.filt <- HTODemux(patient4.filt, assay = "HTO", positive.quantile = posquantile, seed = 100)
# patient4.filt$HTO_classification.global_lpq <- patient4.filt$HTO_classification.global
# patient4.filt$HTO_classification_lpq <- patient4.filt$HTO_classification
patient4.filt$hash.ID.lpq <- factor(patient4.filt$hash.ID,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
table(patient4.filt$HTO_classification.global)
```
...
```{r}
htoexpression.norm.df$hash.ID.hpq <- patient4.filt$hash.ID.hpq
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-12-hpq.svg"),HTOexpr12.norm.hpq)
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-13-hpq.svg"),HTOexpr13.norm.hpq)
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-23-hpq.svg"),HTOexpr23.norm.hpq)

htoexpression.norm.df$hash.ID.lpq <- patient4.filt$hash.ID.lpq
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-12-lpq.svg"),HTOexpr12.norm.lpq)
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-13-lpq.svg"),HTOexpr13.norm.lpq)
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-23-lpq.svg"),HTOexpr23.norm.lpq)
```

UMAP Visualization of HTO Expression

```{r HTO-umap}
patient4.filt <- ScaleData(patient4.filt,
                           assay = "HTO",
                           features = rownames(GetAssayData(patient4.filt,assay = "HTO")))
patient4.filt <- RunPCA(patient4.filt,
                        assay = "HTO", 
                        features = rownames(GetAssayData(patient4.filt,assay = "HTO")), 
                        approx = FALSE,
                        reduction.name = "pca_HTO")
patient4.filt <- RunUMAP(patient4.filt,
                         dims = 1:length(rownames(GetAssayData(patient4.filt,assay = "HTO"))),
                         assay = "HTO", 
...
HTOumap.HTOexpr <- FeaturePlot(patient4.filt,
                               pt.size = 1,
                               reduction = "umap_HTO",
                               features = rownames(GetAssayData(patient4.filt,assay = "HTO"))) +
  theme_jb_nogrid()
patient4.filt$nCount_RNA_log10 <- log10(patient4.filt$nCount_RNA)
patient4.filt$nFeature_RNA_log10 <- log10(patient4.filt$nFeature_RNA)

HTOumap.QC <- FeaturePlot(patient4.filt,
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTO-UMAP-HTOexpr.svg"),HTOumap.HTOexpr,width=5,height=5)
lapply(names(HTOumap.QC), function(x) ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTO-UMAP-",x,".svg"),HTOumap.QC[[x]],width=5,height=5))
HTOumap.HTOexpr
HTOumap.QC
```

```{r}
HTOumap.hashID.hpq <- DimPlot(patient4.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
HTOumap.hashID.lpq <- DimPlot(patient4.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTO-UMAP-hashID-hpq.svg"),HTOumap.hashID.hpq,width=5,height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTO-UMAP-hashID-lpq.svg"),HTOumap.hashID.lpq,width=5,height=5)
...
HTOcounts <- FetchData(object = patient4.filt, vars = c(HTO1,HTO2,HTO3),slot = "counts")
...
patient4.filt$hash.ID.man <- factor(HTOcounts$manuelfilt,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
```
...
```{r}
htoexpression.norm.df$hash.ID.man <- patient4.filt$hash.ID.man
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-12-man.svg"),HTOexpr12.norm.man)
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-13-man.svg"),HTOexpr13.norm.man)
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTOexpr-norm-23-man.svg"),HTOexpr23.norm.man)
```

```{r}
HTOumap.hashID.man <- DimPlot(patient4.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-demux-HTO-UMAP-hashID-man.svg"),HTOumap.hashID.man,width=5,height=5)
...
patient4.filt <- NormalizeData(patient4.filt, 
...
patient4.filt <- FindVariableFeatures(patient4.filt, 
                                      assay = "RNA",
...
allgenes <- rownames(patient4.filt)
patient4.filt <- ScaleData(patient4.filt, 
                           features = allgenes, 
                           do.scale = T,
                           do.center = T,
...
```{r}
patient4.filt <- CellCycleScoring(patient4.filt,
...
```{r}
patient4.filt <- SCTransform(patient4.filt, 
                             ncells = ncol(patient4.filt),
                             assay="RNA", #default
...
patient4.filt <- RunPCA(patient4.filt, 
                        ...
                        features = VariableFeatures(patient4.filt,assay = "SCT"), #default
...
PCAvar.SCT <- patient4.filt@reductions$pca_SCT@stdev^2 / sum(matrixStats::rowVars(as.matrix(patient4.filt@assays$SCT@scale.data)[VariableFeatures(patient4.filt,assay = "SCT"),]))

#PCA plot
PCAplot.SCT <- DimPlot(patient4.filt, reduction = "pca_SCT", dims = c(1,2), group.by = "hash.ID") + 
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-PCA.svg"),PCAplot.SCT, width = 5, height = 5)
# without legends
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-PCA-noleg.svg"),PCAplot.SCT + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-PCA-elbow.svg"),PCAelbowplot.SCT, width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-PCA-elbow-noleg.svg"),PCAelbowplot.SCT + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-PCA-variancesum.svg"),PCAsumplot.SCT, width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-PCA-variancesum-noleg.svg"),PCAsumplot.SCT + theme(legend.position = "none") + labs(title = NULL),width = 5, height = 5)
...
#based on PCA
patient4.filt <- FindNeighbors(patient4.filt, 
                               dims = 1:numberofPCs, 
                               reduction = "pca_SCT",
                               assay = "SCT")
patient4.filt <- FindClusters(patient4.filt, 
                              resolution = 1,
                              random.seed=100)

patient4.filt$seurat_clusters_all_SCT_PCA <- patient4.filt$seurat_clusters
patient4.filt$seurat_clusters <- NULL 
```
...
```{r}
UMAP.SCT.clusPCA <- DimPlot(patient4.filt, 
                            reduction = "umap_SCT", 
                            pt.size = 1, 
                            label = T, 
...
UMAP.SCT.hashIDman <- DimPlot(patient4.filt, 
                              reduction = "umap_SCT", 
                              pt.size = 1, 
                              label = F, 
                              group.by = "hash.ID.man") +
...
UMAP.SCT.QC <- FeaturePlot(patient4.filt, 
...
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-UMAP-clusPCA-noleg.svg"),UMAP.SCT.clusPCA + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-UMAP-hashIDman.svg"),UMAP.SCT.hashIDman, width=5, height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-UMAP-hashIDman-noleg.svg"),UMAP.SCT.hashIDman + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-UMAP-QC.svg"),UMAP.SCT.QC, width=10, height=15)
...
```{r}
UMAP.SCT.marker <- lapply(names(celltypemarkers), function(x) FeaturePlot_markers_comb(patient4.filt,celltypemarkers[[x]],reduction = "umap_SCT"))
names(UMAP.SCT.marker) <- names(celltypemarkers)

lapply(names(UMAP.SCT.marker), function(x) ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-UMAP-marker-",x,".svg"),UMAP.SCT.marker[[x]], width=ifelse(length(celltypemarkers[[x]])>1,10,5), height=5*ceiling(length(celltypemarkers[[x]])/2)))
...
```{r umap-celltype}
Idents(patient4.filt) <- patient4.filt$seurat_clusters_all_SCT_PCA
patient4.filt <- RenameIdents(patient4.filt,
                              "0" = "B-Cells",
                              "1" = "T-Cells",
                              "2" = "T-Cells",
                              "3" = "T-Cells",
...
patient4.filt$CellType <- Idents(patient4.filt)
```

## UMAP

```{r}
UMAP.SCT.celltype <- DimPlot(patient4.filt,
                             reduction = "umap_SCT", 
                             pt.size = 1, 
                             label = T, 
                             group.by = "CellType") + 
  labs(x="UMAP_1",y="UMAP_2") +
  theme_jb_nogrid()
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-UMAP-celltype.svg"),UMAP.SCT.celltype, width=5, height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient4-all-SCT-UMAP-celltype-noleg.svg"),UMAP.SCT.celltype + theme(legend.position = "none") + labs(title = NULL), width=5, height=5)
UMAP.SCT.celltype
```
...
save(file = paste0(OUTPUT,"RData/patient4.RData"),patient4.filt)
#sessionInfo
writeLines(capture.output(sessionInfo()), paste0(OUTPUT,"RData/patient4-preprocessing-sessionInfo.txt"))
```
