---
title: "D19200 - Trajectory analysis cc"
author: "Kai"
...
clusters <- as.character(D19200.tumor$cctraj)
names(clusters) <- names(D19200.tumor$cctraj)
...
for (crv in 1:dim(slingPseudotime(pt))[2]) {
  ...
  ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-crv",crv,".svg"), ptcrv.dimred[[crv]],width = 5,height=5)
  ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-crv",crv,"-pt.svg"), ptcrv.dimred.pt[[crv]],width = 5,height=5)
  
  ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-crv",crv,"-noleg.svg"), ptcrv.dimred[[crv]] + theme(legend.position = "none") + labs(title = NULL),width = 5,height=5)
  ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-crv",crv,"-pt-noleg.svg"), ptcrv.dimred.pt[[crv]] + theme(legend.position = "none") + labs(title = NULL),width = 5,height=5)
}
...
ptcrv.dimred.comb <- ggplot() +
  ...
  labs(x = "principal component 1 (3.9% variance)", y = "principal component 2 (1.2% variance)", title = paste0(red,"combined")) +
  theme_jb_nogrid()

ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-combined-clus.svg"), ptcrv.dimred.comb,width = 5,height=5)
ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-combined-clus-noleg.svg"), ptcrv.dimred.comb + theme(legend.position = "none") + labs(title = NULL),width = 5,height=5)

#colored by pseudotime
ptcrv.comb <- slingPseudotime(pt)[,1]
ptcrv.comb[is.na(ptcrv.comb)] <- slingPseudotime(pt)[,2][is.na(ptcrv.comb)]
ptcrv.comb[is.na(ptcrv.comb)] <- slingPseudotime(pt)[,3][is.na(ptcrv.comb)]
...
ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-combined-pt.svg"), ptcrv.dimred.comb.pt, width = 5,height=5)
ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-combined-pt-noleg.svg"), ptcrv.dimred.comb.pt + theme(legend.position = "none") + labs(title = NULL),width = 5,height=5)
...
for (crv in 1:ncol(slingPseudotime(pt))) {
  ...
  ptcrv.hist[[crv]] <- ggplot(data=data.frame(pt=ptcrv.nona[[crv]],
    ...
    geom_histogram(mapping = aes(x=pt,fill=cluster),
                   ...
                   alpha=0.95,
                   position="stack") +
    coord_cartesian(xlim = c(0,max(unlist(ptcrv.nona)))) +
  ...
  ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-crv",crv,"-density.svg"),ptcrv.hist[[crv]], width = 3.5,height=3.5)
  ggsave(filename = paste0(OUTPUT,"figs-D19200/D19200-cc-tumornofibmono-pt-",red,"-crv",crv,"-density-noleg.svg"),ptcrv.hist[[crv]] + theme(legend.position = "none") + labs(title = NULL), width = 3.5,height=3.5)
}
...
```{r}
#save seurat object
save(file = paste0(OUTPUT,"RData/D19200-cc-tumornofibmono-pseudotimeseurat.RData"),D19200.tumor)
#sessioninfo
writeLines(capture.output(sessionInfo()), paste0(OUTPUT,"RData/10-D19200-cc-tumornofibmono-pseudotime-sessionInfo.txt"))
```
