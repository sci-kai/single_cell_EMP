...
#set output
OUTPUT = "~/tscr/kai/projects/01_SCC_scRNA/results/samples/cohort/patient6/6a/"
```

# Load files

```{r}
data_dir = "~/tscr/Allgemeine Daten/10x Genomics/data/PEI04_A006850061_SCC/cellranger/SID121689/outs/"

#raw data containing gene expression and Hashtag matrices
patient6a.data <- Read10X(paste0(data_dir,"raw_feature_bc_matrix/"))
#Setup Seurat object for gene Expression
patient6a <- CreateSeuratObject(patient6a.data$`Gene Expression`, project = "patient6a")
#Adding HTO data as an independent assay
patient6a[["HTO"]] <- CreateAssayObject(counts = patient6a.data$`Antibody Capture`)
...
patient6a[["percent.mito"]] <- PercentageFeatureSet(patient6a, pattern = "^MT-") 
...
hkgenes.found <- which(toupper(rownames(patient6a)) %in% toupper(hkgenes))
# sum of hk genes for each cell
hk_per_cell <- Matrix::colSums(GetAssayData(patient6a)[hkgenes.found, ] > 0)
patient6a <- AddMetaData(object = patient6a, metadata = hk_per_cell, col.name = "n.exp.hkgenes")
...
barcode_metrics <- data.frame(barcodes = names(patient6a$nCount_RNA),
                              nCount = patient6a$nCount_RNA,
                              nFeature = patient6a$nFeature_RNA,
                              percent.mito = patient6a$percent.mito,
                              n.exp.hkgenes = patient6a$n.exp.hkgenes)

cumplotdata <- data.frame(x=1:1000, y= unlist(lapply(1:1000,function(x) sum(patient6a$nCount_RNA < x) / length(patient6a$nCount_RNA < x))))
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-cumplot-log10.svg"),cumplot.log10)
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsGENE-mito.svg"),UMIvsGENE.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsGENE-hk.svg"),UMIvsGENE.hk)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-HKvsMITO-UMI.svg"),HKvsMITO.UMI)
```
...
patient6a.filt <- subset(patient6a,
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsGENE-filt.svg"),UMIvsGENE.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsGENE-cells-mito.svg"),UMIvsGENE.cells.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsGENE-notcells-mito.svg"),UMIvsGENE.notcells.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsMITO-countfilt-hk.svg"),UMIvsMITO.cf.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsHK-countfilt-mito.svg"),UMIvsHK.cf.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-HKvsMITO-countfilt-UMI.svg"),HKvsMITO.cf.UMI)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsMITO-filt.svg"),UMIvsMITO.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsMITO-cells-hk.svg"),UMIvsMITO.cells.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsMITO-notcells-hk.svg"),UMIvsMITO.notcells.hk)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsHK-filt.svg"),UMIvsHK.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsHK-cells-mito.svg"),UMIvsHK.cells.mito)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-UMIvsHK-notcells-mito.svg"),UMIvsHK.notcells.mito)
```
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-HKvsMITO-filt.svg"),HKvsMITO.filt)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-HKvsMITO-cells-UMI.svg"),HKvsMITO.cells.UMI)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-filtering-HKvsMITO-notcells-UMI.svg"),HKvsMITO.notcells.UMI)
```
...
```{r}
rownames(patient6a.filt@assays$HTO@counts) <- c("6.1","6.2","6.3")
rownames(patient6a.filt@assays$HTO@data) <- c("6.1","6.2","6.3")

#names in HTO matrix
HTO1 <- paste0("hto_",rownames(patient6a.filt@assays$HTO)[1])
HTO2 <- paste0("hto_",rownames(patient6a.filt@assays$HTO)[2])
HTO3 <- paste0("hto_",rownames(patient6a.filt@assays$HTO)[3])
```

## Normalize HTO expression matrix

```{r}
patient6a.filt <- NormalizeData(patient6a.filt, assay = "HTO", normalization.method = "CLR") #centered log-ratio transformation
```
...
htoexpression.norm.df <- FetchData(object = patient6a.filt, vars = c(HTO1,HTO2,HTO3),slot = "data")
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOexpr-norm-123.svg"),HTOexpr123.norm)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOexpr-norm-132.svg"),HTOexpr132.norm)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOexpr-norm-231.svg"),HTOexpr231.norm)
```
...
patient6a.filt.quant <- list()
patient6a.filt.quant.table <- list()
for (quant in seq(0.5,0.99,0.01)) {
  #Demultiplexing
  patient6a.filt.quant[[as.character(quant)]] <- patient6a.filt
  patient6a.filt.quant[[as.character(quant)]] <- HTODemux(patient6a.filt.quant[[as.character(quant)]], assay = "HTO", positive.quantile = quant)
  patient6a.filt.quant.table[[as.character(quant)]] <- table(patient6a.filt.quant[[as.character(quant)]]$HTO_classification.global)
}
rm(patient6a.filt.quant)

demux.df <- data.frame(quantile=names(patient6a.filt.quant.table),
                       Doublet=unlist(lapply(patient6a.filt.quant.table,function(x) x[1])),
                       Negative=unlist(lapply(patient6a.filt.quant.table,function(x) x[2])),
                       Singlet=unlist(lapply(patient6a.filt.quant.table,function(x) x[3])))
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOquant.svg"),HTOquant, width=10)
...
patient6a.filt <- HTODemux(patient6a.filt, assay = "HTO", positive.quantile = 0.99, seed = 100)
# patient6a.filt$HTO_classification.global_hpq <- patient6a.filt$HTO_classification.global
# patient6a.filt$HTO_classification_hpq <- patient6a.filt$HTO_classification
patient6a.filt$hash.ID.hpq <- factor(patient6a.filt$hash.ID,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
table(patient6a.filt$HTO_classification.global)
```

Quality control

```{r}
htoexpression.norm.df$hash.ID.hpq <- patient6a.filt$hash.ID.hpq
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOexpr-norm-12-hpq.svg"),HTOexpr12.norm.hpq)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOexpr-norm-13-hpq.svg"),HTOexpr13.norm.hpq)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOexpr-norm-23-hpq.svg"),HTOexpr23.norm.hpq)
...
```{r HTO-umap}
patient6a.filt <- ScaleData(patient6a.filt,
                            assay = "HTO",
                            features = rownames(GetAssayData(patient6a.filt,assay = "HTO")))
patient6a.filt <- RunPCA(patient6a.filt,
                         assay = "HTO", 
                         features = rownames(GetAssayData(patient6a.filt,assay = "HTO")), 
                         approx = FALSE,
                         reduction.name = "pca_HTO")
patient6a.filt <- RunUMAP(patient6a.filt,
                          dims = 1:length(rownames(GetAssayData(patient6a.filt,assay = "HTO"))),
                          assay = "HTO", 
...
HTOumap.HTOexpr <- FeaturePlot(patient6a.filt,
                               pt.size = 1,
                               reduction = "umap_HTO",
                               features = rownames(GetAssayData(patient6a.filt,assay = "HTO"))) +
  theme_jb_nogrid()
patient6a.filt$nCount_RNA_log10 <- log10(patient6a.filt$nCount_RNA)
patient6a.filt$nFeature_RNA_log10 <- log10(patient6a.filt$nFeature_RNA)

HTOumap.QC <- FeaturePlot(patient6a.filt,
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTO-UMAP-HTOexpr.svg"),HTOumap.HTOexpr,width=5,height=5)
lapply(names(HTOumap.QC), function(x) ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTO-UMAP-",x,".svg"),HTOumap.QC[[x]],width=5,height=5))
HTOumap.HTOexpr
HTOumap.QC
```

```{r}
HTOumap.hashID.hpq <- DimPlot(patient6a.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTO-UMAP-hashID-hpq.svg"),HTOumap.hashID.hpq,width=5,height=5)
...
HTOcounts <- FetchData(object = patient6a.filt, vars = c(HTO1,HTO2,HTO3),slot = "counts")
...
patient6a.filt$hash.ID.man <- factor(HTOcounts$manuelfilt,levels=c("Negative","Doublet",sub("hto_","",c(HTO1,HTO2,HTO3))))
```
...
```{r}
htoexpression.norm.df$hash.ID.man <- patient6a.filt$hash.ID.man
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOexpr-norm-12-man.svg"),HTOexpr12.norm.man)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOexpr-norm-13-man.svg"),HTOexpr13.norm.man)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTOexpr-norm-23-man.svg"),HTOexpr23.norm.man)
```

```{r}
HTOumap.hashID.man <- DimPlot(patient6a.filt,
                              pt.size = 1,
                              reduction = "umap_HTO",
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-demux-HTO-UMAP-hashID-man.svg"),HTOumap.hashID.man,width=5,height=5)
...
patient6a.filt <- NormalizeData(patient6a.filt, 
...
patient6a.filt <- FindVariableFeatures(patient6a.filt, 
                                       assay = "RNA",
...
allgenes <- rownames(patient6a.filt)
patient6a.filt <- ScaleData(patient6a.filt, 
                            features = allgenes, 
                            do.scale = T,
                            do.center = T,
...
```{r}
patient6a.filt <- CellCycleScoring(patient6a.filt,
...
```{r}
patient6a.filt <- SCTransform(patient6a.filt, 
                              ncells = ncol(patient6a.filt),
                              assay="RNA", #default
...
patient6a.filt <- RunPCA(patient6a.filt, 
                         ...
                         features = VariableFeatures(patient6a.filt,assay = "SCT"), #default
...
PCAvar.SCT <- patient6a.filt@reductions$pca_SCT@stdev^2 / sum(matrixStats::rowVars(as.matrix(patient6a.filt@assays$SCT@scale.data)[VariableFeatures(patient6a.filt,assay = "SCT"),]))

#PCA plot
PCAplot.SCT <- DimPlot(patient6a.filt, reduction = "pca_SCT", dims = c(1,2), group.by = "hash.ID") + 
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-PCA.svg"),PCAplot.SCT, width = 5, height = 5)
# without legends
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-PCA-noleg.svg"),PCAplot.SCT + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-PCA-elbow.svg"),PCAelbowplot.SCT, width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-PCA-elbow-noleg.svg"),PCAelbowplot.SCT + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-PCA-variancesum.svg"),PCAsumplot.SCT, width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-PCA-variancesum-noleg.svg"),PCAsumplot.SCT + theme(legend.position = "none") + labs(title = NULL),width = 5, height = 5)
...
#based on PCA
patient6a.filt <- FindNeighbors(patient6a.filt, 
                                dims = 1:numberofPCs, 
                                reduction = "pca_SCT",
                                assay = "SCT")
patient6a.filt <- FindClusters(patient6a.filt, 
                               resolution = 1,
                               random.seed=100)

patient6a.filt$seurat_clusters_all_SCT_PCA <- patient6a.filt$seurat_clusters
patient6a.filt$seurat_clusters <- NULL 
```
...
```{r}
UMAP.SCT.clusPCA <- DimPlot(patient6a.filt, 
                            reduction = "umap_SCT", 
                            pt.size = 1, 
                            label = T, 
...
UMAP.SCT.hashIDman <- DimPlot(patient6a.filt, 
                              reduction = "umap_SCT", 
                              pt.size = 1, 
                              label = F, 
                              group.by = "hash.ID.man") +
...
UMAP.SCT.QC <- FeaturePlot(patient6a.filt, 
...
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-UMAP-clusPCA-noleg.svg"),UMAP.SCT.clusPCA + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-UMAP-hashIDman.svg"),UMAP.SCT.hashIDman, width=5, height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-UMAP-hashIDman-noleg.svg"),UMAP.SCT.hashIDman + theme(legend.position = "none") + labs(title = NULL), width = 5, height = 5)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-UMAP-QC.svg"),UMAP.SCT.QC, width=10, height=15)
...
```{r}
UMAP.SCT.marker <- lapply(names(celltypemarkers), function(x) FeaturePlot_markers_comb(patient6a.filt,celltypemarkers[[x]],reduction = "umap_SCT"))
names(UMAP.SCT.marker) <- names(celltypemarkers)

lapply(names(UMAP.SCT.marker), function(x) ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-UMAP-marker-",x,".svg"),UMAP.SCT.marker[[x]], width=ifelse(length(celltypemarkers[[x]])>1,10,5), height=5*ceiling(length(celltypemarkers[[x]])/2)))
...
```{r umap-celltype}
Idents(patient6a.filt) <- patient6a.filt$seurat_clusters_all_SCT_PCA
patient6a.filt <- RenameIdents(patient6a.filt,
                               "0" = "T-Cells",
                               "1" = "B-Cells",
                               "2" = "T-Cells",
                               "3" = "T-Cells",
...
patient6a.filt$CellType <- Idents(patient6a.filt)
```

## UMAP

```{r}
UMAP.SCT.celltype <- DimPlot(patient6a.filt,
                             reduction = "umap_SCT", 
                             pt.size = 1, 
                             label = T, 
                             group.by = "CellType") + 
  labs(x="UMAP_1",y="UMAP_2") +
  theme_jb_nogrid()
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-UMAP-celltype.svg"),UMAP.SCT.celltype, width=5, height=5)
ggsave(filename = paste0(OUTPUT,"figs/patient6a-all-SCT-UMAP-celltype-noleg.svg"),UMAP.SCT.celltype + theme(legend.position = "none") + labs(title = NULL), width=5, height=5)
UMAP.SCT.celltype
```
...
save(file = paste0(OUTPUT,"RData/patient6a.RData"),patient6a.filt)
#sessionInfo
writeLines(capture.output(sessionInfo()), paste0(OUTPUT,"RData/patient6a-preprocessing-sessionInfo.txt"))
```
